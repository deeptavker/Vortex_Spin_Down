...
temp_a = load('a10_a_output/a10_a_0.npz')['arrays']['fluid']
...
for fname in get_files('deep_output/'):
    pa = load(fname)
    u = pa['arrays']['fluid'].u
    v = pa['arrays']['fluid'].v
    vmax = np.max((u**2 + v**2)**0.5)
...
plt.plot(time_a[:21], vmax_a[:21])
plt.plot(time_b, vmax_b)
plt.plot(time_c, vmax_c)
plt.title('(WCSPH) Variation of normalized velocity with time')
plt.xlabel('Time(s)')
plt.ylabel('Normalized Velocity')
plt.legend(['C = 1.25 (WCSPH)', 'C = 2.5 (WCSPH)', 'C = 5 (WCSPH)'])
plt.savefig('results_WCSPH_VMAX_1.png')
plt.clf()
plt.plot(time_a[:21], KE_a[:21])
plt.plot(time_b, KE_b)
plt.plot(time_c, KE_c)
plt.title('(WCSPH) Variation of Average Kinetic Energy with time')
plt.xlabel('Time(s)')
plt.ylabel('Normalized Average Kinetic Energy')
plt.legend(['C = 1.25 (WCSPH)', 'C = 2.5 (WCSPH)', 'C = 5 (WCSPH)'])
plt.savefig('results_WCSPH_KE_1.png')
